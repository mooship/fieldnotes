...
**Design system:** Rose Pine colour palette throughout. Light mode uses Rose Pine Dawn (`--paper` #faf4ed, `--ink` #575279) with a Pine accent (`--moss` #286983); dark mode uses Rose Pine Moon (`--paper` #232136, `--ink` #e0def4, `--moss` #3e8fb0). Single accent for links, tags, and hover states — no section-specific colours, no ornaments. The design relies on typography, whitespace, and one restrained accent.
