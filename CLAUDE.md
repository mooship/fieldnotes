...
The production domain (`timothybrits.co.za`, `www.timothybrits.co.za`) is still a Custom Domain on the old Cloudflare Pages project as of this migration — it has **not** moved to the Worker yet. A `wrangler.jsonc`-driven cutover (either the classic `routes` + `custom_domain: true` shape, or a top-level `domains` array) was tried and didn't propagate through this project's build pipeline: `@astrojs/cloudflare` deploys from a `dist/server/wrangler.json` generated by `@cloudflare/vite-plugin`, and neither shape made it into that generated config in testing. Separately, the apex domain has pre-existing DNS records that Cloudflare's Custom Domain API refuses to take over automatically (`Hostname already has externally managed DNS records`). So the domain move has to happen by hand in the Cloudflare dashboard (Workers & Pages → fieldnotes → Settings → Domains & Routes), and **the old Pages project must not be deleted until that move is done and verified** — deleting it first would take the live site down.
